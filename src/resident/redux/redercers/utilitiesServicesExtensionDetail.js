import _ from 'lodash';
import {
    UTILITIES_SERVICES_EXTENSION_DETAIL_RESET_BY_KEY,
    UTILITIES_SERVICES_EXTENSION_DETAIL_REFRESHING,
    UTILITIES_SERVICES_EXTENSION_DETAIL_REQUEST,
    UTILITIES_SERVICES_EXTENSION_DETAIL_SUCCESS,
    UTILITIES_SERVICES_EXTENSION_DETAIL_FAILURE,
    SERVICES_EXTENSION_RATE_REQUEST,
    SERVICES_EXTENSION_RATE_FAILURE,
    SERVICES_EXTENSION_RATE_SUCCESS, 
    SERVICES_EXTENSION_FEEDBACK_REQUEST,
    SERVICES_EXTENSION_FEEDBACK_SUCCESS,
    SERVICES_EXTENSION_FEEDBACK_FAILURE
} from '../../actions/actionTypes';


const INITIAL_STATE = {
    initComponent: false,
    isLoading: false,
    error: null,
    errorResponse: null,
    data: null,
    isRefreshing: false,
    errorProgress: null
};

export default (state = INITIAL_STATE, action) => {
    switch (action.type) {

        case UTILITIES_SERVICES_EXTENSION_DETAIL_RESET_BY_KEY: {
            if (action.payload.key === 'state') {
                return INITIAL_STATE;
            }
            const newValues = state[action.payload.key];
            return {
                ...state,
                [action.payload.key]: action.payload.path && !_.isEmpty(action.payload.path) ? _.set(newValues, action.payload.path, action.payload.value) : action.payload.value
            };
        }
        case UTILITIES_SERVICES_EXTENSION_DETAIL_REQUEST: {
            return {
                ...state,
                initComponent: true,
                isLoading: true,
                error: INITIAL_STATE.error
            }
        }
        case UTILITIES_SERVICES_EXTENSION_DETAIL_SUCCESS: {
            return {
                ...state,
                initComponent: false,
                isLoading: false,
                isRefreshing: false,
                data: action.payload.data,

            };
        }
        case UTILITIES_SERVICES_EXTENSION_DETAIL_FAILURE: {
            return {
                ...state,
                initComponent: false,
                isLoading: false,
                isRefreshing: false,
                error: { hasError: true, message: action.payload.data }
            }
        }
        case SERVICES_EXTENSION_FEEDBACK_REQUEST: {
            return {
                ...state,
                isLoading: true,
                errorResponse: INITIAL_STATE.errorResponse
            }
        }
        case SERVICES_EXTENSION_FEEDBACK_FAILURE: {
            return {
                ...state,
                isLoading: false,
                errorResponse: { hasError: true, status: action.payload.status, statusText: action.payload.statusText }
            }
        }
        case SERVICES_EXTENSION_FEEDBACK_SUCCESS: {
            const newHis = [action.payload.dataHis, ...state.data.historys]
            return {
                ...state,
                isLoading: false,
                data: { ...state.data, historys: newHis },
                errorResponse: { hasError: false }
            };
        }
        case SERVICES_EXTENSION_RATE_REQUEST: {
            return {
                ...state,
                isLoading: true,
                errorProgress: INITIAL_STATE.errorProgress
            }
        }
        case SERVICES_EXTENSION_RATE_SUCCESS: {
            return {
                ...state,
                isLoading: false,
                errorProgress: { hasError: false }
            }
        }
        case SERVICES_EXTENSION_RATE_FAILURE: {
            return {
                ...state,
                isLoading: false,
                errorProgress: { hasError: true, message: action.payload.data }
            }
        }

        default: return state;
    }
};
