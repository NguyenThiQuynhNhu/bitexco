...
import { View, Text, StyleSheet, TouchableOpacity, Image, FlatList, ScrollView, Alert, ActivityIndicator, Platform } from 'react-native';
...
import { loadDataHandle, resetStateByKey, refreshDataHandle } from '../../actions/utilities';
import { loadDataHandle as loadDataHandleBasic, refreshDataHandle2 as refreshDataHandleBasic, resetStateByKey as resetStateByKeyBasic, deleteServicedHandle } from '../../actions/utilitiesServicesBasic'
import { loadDataHandle as loadDataHandleEx, refreshDataHandle2 as refreshDataHandleEx, resetStateByKey as resetStateByKeyEx, deleteServiceExdHandle } from '../../actions/utilitiesServicesExtension'
...
const Banner = ({ text, onPress }) => {
    return (
        <View style={{ justifyContent: 'center', alignItems: 'center' }}>
            <Image source={banner_utility} style={{ width: '100%', height: 100 }} resizeMode="stretch" />
            <TouchableOpacity onPress={onPress} style={{ alignItems: 'center', paddingVertical: 10, width: '50%', backgroundColor: 'rgba(255, 255, 255,0.8)', borderRadius: 45, position: 'absolute' }}>
                <Text>{text}</Text>
            </TouchableOpacity>
        </View>
    )
...
const Group = ({ text, onPress }) => {
    return (
        <View style={{ justifyContent: 'center', alignItems: 'center', height: 50, backgroundColor: 'rgba(255, 255, 255,0.8)', borderRadius: 45 }}>
            <TouchableOpacity onPress={onPress} style={{ alignItems: 'center', width: '50%', position: 'absolute' }}>
                <Text style={{ fontSize: fontsize.small, color: colors.appTheme, fontWeight: 'bold', }}>{text}</Text>
            </TouchableOpacity>
        </View>
    )
}
...
class UtilityScreen extends Component {
    ...
    componentDidMount() {
        this.props.loadDataHandle({ towerId: this.props.towerId, langId: this.props.langId })
        //basic
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.towerId && nextProps.towerId !== this.props.towerId) {
            this.props.loadDataHandle({ towerId: nextProps.towerId, langId: this.props.langId })
        }

        //basic
        if (nextProps.isRefreshingBasic && nextProps.isRefreshingBasic !== this.props.isRefreshingBasic) {
            const data = {
        ...
        if (nextProps.errorDeBasic && nextProps.errorDeBasic !== this.props.errorDeBasic) {
            if (!nextProps.errorDeBasic.hasError) {
                //this.props.navigation.goBack();
                // this.refs.toast.show('Hủy thành công!', DURATION.LENGTH_LONG);
                this.props.refreshDataHandleBasic()
            } else {
                this.refs.toast.show(nextProps.errorDeBasic.message, DURATION.LENGTH_LONG);
            }
        }
        ...
        if (nextProps.isRefreshingEx && nextProps.isRefreshingEx !== this.props.isRefreshingEx) {
            const data = {
                towerId: this.props.towerId,
                statusId: 0,
                keyword: this.state.isApplySearchKey ? this.state.searchKey : '',
    ...
    renderItem = ({ item, index }) => {
        ...
        return (
            <TouchableOpacity
                onPress={() => this.props.canNav ? (typeId === 0 ? this.props.navigation.navigate('basicDetail', item) : this.props.navigation.navigate('services', item)) : null}
                style={{
                    margin: 10, width: (Screen.width / 3) - 20,
    ...
    renderContent() {
        ...
        if (emptyData || ( data && (data.filter(o => o.typeId == type)).length == 0)) {
            return (
                <ErrorContent title={Strings.app.emptyData} onTouchScreen={() => this.props.loadDataHandle({ towerId: this.props.towerId, langId: this.props.langId })} />
            )
        }
        if (initComponent || isLoading) {
            ...
            return (
                ...
                <ErrorContent title={Strings.app.error} onTouchScreen={() => this.props.loadDataHandle({ towerId: this.props.towerId, langId: this.props.langId })} />
            )
        }
        else {
            if (data) {
                ...
                return (
                    ...
                    <ScrollView style={{}} showsVerticalScrollIndicator={false}>
                        ...
                        <Text style={{
                            ...
                            letterSpacing: 0,
                            textAlign: 'left',
                            color: "#282828", marginBottom: 0, marginHorizontal: 20
                        }}>
                            {this.state.basic ? Strings.setting.utilityRegister : Strings.setting.serviceRegister}
                        </Text>
                        { this.state.basic ? this.renderContentBasic() : this.renderContentEx()}
    ...
    deleteService(id) {
        Alert.alert(
            ...
            [
                ...
                { text: "OK", onPress: () => { return this.props.deleteServicedHandle({ bookingId: id, description: 'Hủy phiếu' }) } }
            ]
        );
        
    }
    renderContentBasic() {
        const { data, emptyDataBasic, errorBasic, initListBasic, dataBasic, isRefreshingBasic, isLoadingBasic } = this.props;
        ...
        if (emptyDataBasic) {
            return <ErrorContent title={Strings.app.emptyData} onTouchScreen={() => this.props.refreshDataHandleBasic()} />
        }
        if (errorBasic && errorBasic.hasError) {
            return (
                <ErrorContent title={Strings.app.error} onTouchScreen={() => this.props.refreshDataHandleBasic()} />
            )
        }
        return (
            <FlatList
                ...
                onEndReached={() => {
                    if (!this.onEndReachedCalledDuringMomentum && !this.props.outOfStockBasic && this.props.currentPageBasic > 0 && !this.props.isLoadingBasic) {
                        const data = {
                            towerId: this.props.towerId,
                            statusId: 0,
                            keyword: this.state.isApplySearchKey ? this.state.searchKey : '',
    ...
    renderItemBasic = ({ item }) => {
        ...
        deleteService={ (id) => this.deleteService(id)}
            ...
            onPress={() => this.props.navigation.navigate('serviceBasicDetailResident', item)}
        />
    }
    renderContentEx() {
        const { data, emptyDataEx, errorEx, initListEx, dataEx, isRefreshingEx, outOfStockEx, refreshDataHandleEx, loadDataHandleEx, isLoadingEx } = this.props;
        if (!data && data.length == 0) return null
        // if (isLoadingEx) {
        //     //('initList')
        //     return (
        ...
        // }
        if (emptyDataEx) {
            return <ErrorContent title={Strings.app.emptyData} onTouchScreen={() => {this.props.refreshDataHandleEx(), this.setState({ dataStatus: this.props.dataStatus })}} />
        }
        if (errorEx && errorEx.hasError) {
            return (
                <ErrorContent title={Strings.app.error} onTouchScreen={() => {this.props.refreshDataHandleEx(), this.setState({ dataStatus: this.props.dataStatus })}} />
            )
        }
        return (
            <FlatList
                ...
                onMomentumScrollBegin={() => { this.onEndReachedCalledDuringMomentum = false }}
                onEndReached={() => {
                    if (!this.onEndReachedCalledDuringMomentum && !this.props.outOfStockEx && this.props.currentPageEx > 0 && !this.props.isLoadingEx) {
                        const data = {
                            towerId: this.props.towerId,
                            statusId: 0,
                            keyword: this.state.isApplySearchKey ? this.state.searchKey : '',
    ...
    renderItemEx = ({ item }) => {
        ...
        return <ListItemEx item={item}
        deleteService={ (id) => this.deleteServiceEx(id)}
            onPress={() => this.props.navigation.navigate('serviceExtensionDetailResident', item)}
        />
    }
    deleteServiceEx(id) {
        Alert.alert(
            ...
            [
                ...
                { text: "OK", onPress: () => { return this.props.deleteServiceExdHandle({ bookingId: id, description: 'Hủy phiếu' }) } }
            ]
        );
            ...
            <View style={styles.container}>
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginHorizontal: 20, marginTop: -10, alignItems: 'center', padding: 10 }}>
                    <TouchableOpacity
                        ...
                        }}>{Strings.serviceBasic.title}</Text>
                        <View style={{
                            marginTop: 5,
                            borderRadius: 45,
                            minWidth: 25,
                            ...
                            }}>{!this.props.data ? '0' : (this.props.data.filter(o => o.typeId == 0)).length}</Text>
                        </View>
                        <View style={{
                    ...
                    <TouchableOpacity
                        ...
                        <Text style={{
                            ...
                            textAlign: "center",
                            color: this.state.basic ? '#c8c8c8' : "#3d3d3d"
                        }}>{Strings.serviceExtension.title}</Text>
                        <View style={{
                            marginTop: 5,
                            borderRadius: 45,
                            minWidth: 25,
                            ...
                            <Text style={{
                                ...
                                textAlign: "center",
                                color: this.state.basic ? '#c8c8c8' : '#a3cd80'
                            }}>{!this.props.data ? '0' : (this.props.data.filter(o => o.typeId == 1)).length}</Text>
                        </View>
                        <View style={{
                ...
                <Spinner visible={this.props.isLoadingDeBasic || this.props.isLoadingDeEx} color={colors.primaryKeyColor} />
            </View>
        );
    }
    basicClick() {
...
export default connect(mapStateToProps, mapActionToProps)(UtilityScreen);
