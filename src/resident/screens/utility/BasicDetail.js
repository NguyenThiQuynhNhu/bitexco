...
import { View, Text, StyleSheet, Linking, ActivityIndicator, ScrollView, TouchableOpacity, Modal, TextInput, Platform } from 'react-native';
import { WebView } from 'react-native-webview';
import { connect } from 'react-redux';
import moment from 'moment';
import Toast, { DURATION } from 'react-native-easy-toast';
import { loadDataHandle, resetStateByKey, createBookingHandle, refreshDataHandle } from '../../actions/utilitiesBasicDetail';
...
class ServicesScreen extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.errorProgress && nextProps.errorProgress !== this.props.errorProgress) {
            if (nextProps.errorProgress.hasError) {
                this.refs.toast.show(nextProps.errorProgress.message, DURATION.LENGTH_LONG);
            } else {
                this.setState({ showModal: false }, () => this.refs.toast.show(Strings.message.bookSuccess, DURATION.LENGTH_LONG))
            }
        }
    ...
    renderContent() {
        const { initComponent, isLoading, error, data, createBookingHandle } = this.props;
        if (initComponent || isLoading) {
            ...
            return (
                <View style={{ flex: 1 }}>
                    <View style={{ width: Screen.width - 40, marginHorizontal: 20, height: 194, borderRadius: 12 }}>
                        <ImageProgress
                            source={{ uri: logo }}
                            style={{ height: 194, width: Screen.width - 40, borderColor: colors.grayBorder, borderRadius: 12, backgroundColor: colors.gray2 }}
                        />
                    </View>
                    <Text style={{
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginHorizontal: 20, marginTop: 15 }}>
                        <Text style={{
                            ...
                            textAlign: "left",
                            color: "#282828"
                        }}>{Strings.serviceBasic.maximumAmountPeople}</Text>
                        <Text style={{
                            fontFamily: "Inter-SemiBold",
                            fontSize: 14,
                            fontWeight: "600",
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginHorizontal: 20, marginTop: 15 }}>
                        <Text style={{
                            ...
                            textAlign: "left",
                            color: "#282828"
                        }}>{Strings.serviceBasic.deposit}</Text>
                        <Text style={{
                            fontFamily: "Inter-SemiBold",
                            fontSize: 14,
                            fontWeight: "600",
                            ...
                            textAlign: "left",
                            color: "#ff3d00"
                        }}>{deposit.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')} <Text style={{fontFamily: "Inter-SemiBold", fontSize: 12, color: "#a0a0a0"}}>VNĐ</Text></Text>
                    </View>
                    ...
                    {/* <WebView
                        ...
                        ref="webView"
                    /> */}
                    <View style={{ height: '60%', width: Screen.width - 40, marginHorizontal: 20, marginTop: 15, paddingBottom: 80 }}>
                        <WebView
                            source={{ html: `<html xmlns="http://www.w3.org/1999/xhtml"><head><meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1" /></head><style>img{ width: 100%  !important; height:auto !important} iframe{ width: 100%  !important; height:auto !important; background: '#333' !important}</style><body>${description}</body></html>` }}
                    ...
                    <PrimaryButton text={Strings.serviceBasicBooking.book} onPress={() => this.props.navigation.navigate('basicBooking', { data, name: this.props.navigation.state.params.name })}
                        style={{ position: 'absolute', bottom: 10, borderRadius: 45, margin: 20, width: 150, alignSelf: 'center' }} />
                </View>
            )
        }
    }
        ...
        if (!this.state.isLoadDesign) {
            return (<View style={styles.container}>
                ...
                <NavBar
                    ...
                    body={<Text style={{ alignSelf: 'center', color: '#fff', fontSize: fontsize.medium }}>{this.props.navigation.state.params.name}</Text>}
                />
                <View
                    style={{
                        paddingVertical: 20,
        ...
        return (
            <View style={styles.container}>
                ...
                <NavBar
                    ...
                    rightView={
                        <TouchableOpacity
                            onPress={() => this.props.navigation.navigate('serviceBasicResident', { id: data.id, name: this.props.navigation.state.params.name })}
                            style={{ padding: 10 }}>
...
export default connect(mapStateToProps, mapActionToProps)(ServicesScreen);
