...
import { loadDataHandle, resetStateByKey, refreshDataHandle } from '../../actions/utilitiesServices';
    ...
    renderItem = ({ item }) => {
        ...
        return (
            <TouchableOpacity
                ...
                style={{
                    marginHorizontal: 20, marginBottom: 20, backgroundColor: '#fff', borderRadius: 12,
                ...
                <ImageProgress
                    source={{ uri: logo }}
                    style={{ height: 150, width: '100%', borderTopRightRadius: 12, borderTopLeftRadius: 12 }}
                />
                <Text style={{
                    ...
                    letterSpacing: 0,
                    textAlign: "left",
                    color: "#3d3d3d", margin: 10
                }}>{name}</Text>
                <View style={{ flexDirection: 'row', paddingHorizontal: 10, paddingBottom: 10, justifyContent: 'space-between', alignItems: 'center' }} >
                    <View style={{}}>

                        {
                            price > 0 ? <View style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center' }}>
                                <Text style={{
                                    ...
                                    textAlign: "right",
                                    color: "#ff624d"
                                }}>{price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}</Text>
                                <Text style={{
                                    fontFamily: "Inter-Medium",
                                    fontSize: 11,
                                    fontWeight: "500",
                    ...
                    <View>
                        <PrimaryButton text={Strings.serviceExtension.bookService} onPress={() =>
                            this.setState({
                                showModal: true,
                                dataRequest: {
                                    departmentId,
        ...
        return (
            <View style={styles.container}>
                ...
                <NavBar
                    ...
                    fontWeight: "bold",
                    textAlign: "center",
                    color: "#ffffff" }}>{this.props.navigation.state.params.name}</Text>}
                    rightView={
                        <TouchableOpacity
                ...
                />
                {this.renderContent()}
                {this.state.dataRequest && <BookingModal
