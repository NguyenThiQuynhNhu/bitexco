...
import { loadDataHandle, resetStateByKey, createBookingHandle } from '../../actions/utilitiesServicesDetail';
        ...
        if (initComponent || isLoading) {
            ...
            return (
                <View style={{ flex: 1, marginTop: -20 }}>
                    <ImageProgress
                        source={{ uri: logo }}
                        style={{ height: 194, width: Screen.width - 40, margin: 20, borderColor: colors.grayBorder, borderRadius: 12 }}
                    />
                    {/* <WebView
                        style={{ flex: 1 }}
                    ...
                    <View style={{ flexDirection: 'row', justifyContent: 'flex-start', alignItems: 'center', marginHorizontal: 20, marginTop: 20 }}>
                        ...
                        <Text style={{
                            ...
                            textAlign: "left",
                            color: "#ff3d00"
                        }}>{deposit.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}</Text>
                        <Text style={{
                            fontFamily: "Inter-SemiBold",
                            fontSize: 12,
                            fontWeight: "600",
                    ...
                    <View style={{ height: '60%', width: Screen.width, paddingHorizontal: 20, paddingBottom: 60 }}>
                        <WebView
                            source={{ html: `<html xmlns="http://www.w3.org/1999/xhtml"><head><meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1" /></head><style>img{ width: 100%  !important; height:auto !important} iframe{ width: 100%  !important; height:auto !important}</style><body>${description}</body></html>` }}
                    ...
                    />
                    <PrimaryButton text={Strings.serviceExtension.bookService} onPress={() => { this.setState({ showModal: true }) }} style={{ position: 'absolute', bottom: 20, alignSelf: 'center', borderRadius: 12 }} />
                </View>
            )
        }
    }
        ...
        if (!this.state.isLoadDesign) {
            return (<View style={styles.container}>
                ...
                <NavBar
                    ...
                    body={<Text style={{ alignSelf: 'center', color: '#fff', fontSize: fontsize.larg }}>{this.props.navigation.state.params.name}</Text>}
                />
                <View
                    style={{
                        paddingVertical: 20,
        ...
        return (
            <View style={styles.container}>
                ...
                <NavBar
                    ...
                    rightView={
                        <TouchableOpacity
                            onPress={() => this.props.navigation.navigate('serviceExtensionResident', { id: data.id, name: this.props.navigation.state.params.name })}
                            style={{ padding: 10 }}>
                            <Icon name='history'
                                size={24}
                                color='#fff'
