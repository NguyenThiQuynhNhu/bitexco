...
import { loadDataHandle, refreshDataHandle, resetStateByKey } from '../../../../actions/fee';
...
class Fee extends Component {
    ...
    renderItem = ({ item, index }) => {
        return <ListItem
            ...
            item={item}
            onPress={() => this.props.canNavigate ? this.props.navigation.navigate('feeDetail', item) : null}
        />
    }
    renderContent() {
        ...
        return (
            <View style={styles.container}>
                ...
                <View
                    ...
                    <Text>{Strings.fee.total}</Text>
                    <Text style={{ color: colors.appTheme, fontSize: fontsize.medium }}>{`${this.props.total}`.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',')}</Text>
                </View>
                <View style={{
                    paddingHorizontal: 10,
                    flex: 1
                }}>
                    <RowItem title={Strings.fee.detail.toLocaleUpperCase()} styleTitle={{ fontSize: fontsize.medium }} value={`${Strings.fee.unit}: VNĐ`} />
                    {this.renderContent()}
                </View>
...
export default connect(mapStateToProps, mapActionToState)(Fee);
