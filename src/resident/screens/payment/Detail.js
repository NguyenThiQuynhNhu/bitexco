...
class PaymentDetailScreen extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (
      nextProps &&
      nextProps.errorLogin &&
      nextProps.errorLogin.hasError &&
      this.props.errorLogin !== nextProps.errorLogin
    ) {
      Alert.alert(
        "Thông báo",
        "Hết phiên sử dụng, bạn cần đăng nhập lại để tiếp tục!",
        [
    ...
    if (
      nextProps &&
      nextProps.errorSavePayment &&
      this.props.errorSavePayment !== nextProps.errorSavePayment
    ) {
      if (nextProps.errorSavePayment.status === 200) {
        this.setState({ isLoading: false });
        this.props.navigation.navigate("ewallet", {
          urlPaymentViettel: this.state.urlPaymentViettel,
          trans_amount: this.state.trans_amount,
    ...
    return (
      ...
      >
        <FeildText
          ...
          nameStyle={{ color: "#000" }}
          textStyle={{}}
          text={amount.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " đ"}
        />
        <Text
          numberOfLines={2}
          lineBreakMode="tail"
    ...
    return (
      <View style={{ flex: 1, marginBottom: responsive.h(5) }}>
        <FeildText
          ...
          text={
            thanhTien.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " đ"
          }
        />
        <FeildText
          name={Strings.serviceExtension.price}
          nameStyle={{}}
          textStyle={{ color: "#000" }}
          text={donGia.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " đ"}
        />
        <FeildText
          name={Strings.payment.number}
          nameStyle={{}}
          textStyle={{ color: "#000" }}
          text={soLuong}
        />
  ...
  getUrlViettelPayment() {
    ...
    if (
      ...
      access_code == null ||
      hash_key == null ||
      (ma_doi_tac != null && ma_doi_tac == "") ||
      (ten_doi_tac != null && ten_doi_tac == "") ||
      (access_code != null && access_code == "") ||
      (hash_key != null && hash_key == "")
    ) {
      Alert.alert(
        "Thông báo",
      ...
      var order_id =
        this.props.departmentId + "_" + moment().format("YYYYMMDDHHmmss");
      ...
      console.log(
        Base64.stringify(CryptoJS.HmacSHA1(order_id, this.props.user.hash_key))
      );
      let hmacSHA1 = CryptoJS.HmacSHA1(
        toHashCheckSum,
        this.props.user.hash_key
      );
      ...
      this.setState({
        trans_amount: amountIncurred,
        order_id: order_id,
        billcode: billcode,
        urlPaymentViettel: _url,
      });
  ...
  renderButtonPayment() {
    ...
    if (data !== null) {
      if (
        !data.isPaid &&
        this.props.billCode != null &&
        data.isUseEWallet &&
        this.props.user.ma_doi_tac &&
        this.props.user.ma_doi_tac.trim != ""
      ) {
        return (
          <View
            style={{
        ...
        >
          ...
          >
            ...
            >
              <FeildText
                style={{}}
                name={Strings.payment.contractNo}
                text={contractNo}
                textStyle={{ color: colors.appTheme, fontWeight: "bold" }}
              />
              <FeildText
              ...
              />
              <FeildText
                name={Strings.payment.dealine}
                text={dealine}
                textStyle={{ color: "#f2c94c", fontWeight: "bold" }}
              />
              <FeildText
                name={Strings.payment.status}
                textStyle={{
                  color: isPaid ? colors.appTheme : "#eb5757",
                  fontWeight: "bold",
                }}
                text={
                  isPaid
            ...
            >
              ...
              <FeildText
                name={Strings.payment.occursDuring}
                text={
                  amountIncurred
                    .toString()
                    .replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " đ"
              ...
              />
              <FeildText
                name={Strings.payment.paid}
                text={
                  amountPaid.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") +
                  " đ"
                }
              />
              <FeildText
