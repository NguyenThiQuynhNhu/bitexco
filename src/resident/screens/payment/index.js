...
import { View, StyleSheet, Text, FlatList, ActivityIndicator, TouchableOpacity, processColor, Platform } from 'react-native';
...
import { loadDataHandle, resetStateByKey, refreshDataHandle } from '../../actions/fee';
...
class PaymentScreen extends Component {
    ...
    renderItem = ({ item }) => {
        ...
        if (isPaid == this.state.paid) {
            return <ListItem
                onPress={() => this.props.navigation.navigate('paymentDetail', { time })}
            ...
            <View style={{ flex: 1 }}>
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginHorizontal: 20, marginTop: -10, alignItems: 'center', padding: 10 }}>
                    <TouchableOpacity
                        ...
                        }}>{Strings.payment.unpaidStatus}</Text>
                        <View style={{
                    ...
                    <TouchableOpacity
                        ...
                        <Text style={{
                            ...
                            textAlign: "center",
                            color: !this.state.paid ? '#c8c8c8' : "#3d3d3d"
                        }}>{Strings.payment.paidStatus}</Text>
                        <View style={{
                            width: Platform.isPad ? 64 : 44,
                            height: 3,
                            borderRadius: 4,
    ...
    demo() {
        ...
        const toHashCheckSum = 'd41d8cd98f00b204e9800998ecf8427e7374a0c0b8785ce841010011e2a3074c' + billcode + command + merchant_code + order_id + trans_amount + version

        let demo = CryptoJS.HmacSHA1(toHashCheckSum, "d41d8cd98f00b204e9800998ecf8427e067c8498698af55ffdfe1266a1583f03")
    ...
    renderContent() {
        ...
        if (emptyData || (data && (data.filter(item => item.isPaid == this.state.paid)).length == 0)) {
