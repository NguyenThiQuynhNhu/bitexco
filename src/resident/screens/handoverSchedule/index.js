...
import { View, Text, StyleSheet, TouchableOpacity, FlatList, ScrollView, ActivityIndicator, Platform, SafeAreaView, TouchableWithoutFeedback, Modal } from 'react-native';
...
import { loadDataHandle, resetStateByKey, refreshDataHandle, loadDataDayListHandle, handoverSetProps } from '../../actions/handoverSchedule';
...
class HandoverScheduleScreen extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            date: this.props.navigation.state.params != undefined && this.props.navigation.state.params.date ? this.props.navigation.state.params.date : new Date(),
            itemSelected: null,
            showConfirm: false,
            f: false
        };
    ...
    componentDidMount() {
        ...
        //     })
        // }
        const towerCustomer = this.props.user.towerCustomers.filter(id => id.idTower == this.props.towerId);
        // const data = {
        //     makh: towerCustomer[0].idCustomer
        // }
        // console.log('nhu2')
        // this.props.loadDataHandle(data);
        this.props.loadDataDayListHandle({ date: getDateApi(this.state.date), makh: towerCustomer[0].idCustomer })
    }
    componentWillUpdate(nextProps, nextState) {
        ...
        if (nextProps.isRefreshingSchedul && nextProps.isRefreshingSchedul !== this.props.isRefreshingSchedul) {
            const data = {
                makh: towerCustomer[0].idCustomer
            }
            this.props.loadDataHandle(data);
        ...
        if (nextState.date && nextState.date !== this.state.date) {
            this.props.loadDataDayListHandle({ date: getDateApi(nextState.date), makh: towerCustomer[0].idCustomer })
        }
    ...
    renderContent = () => {
        const { emptyData, error, dataDayList, isRefreshing, isLoading } = this.props;
        if (isLoading) {
            ...
            return (
                <View style={{
                    ...
                    <View style={{
                        ...
                        borderTopStartRadius: 50, flex: 1, justifyContent: 'center', alignItems: 'center'
                    }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginBottom: 10 }}>
                            <MyIcon name="calendar" color="#000" size={20} />
                            <Text style={{ fontWeight: 'bold', color: '#000', marginLeft: 10, fontSize: fontsize.small }}>{moment(this.state.date).format('DD/MM/YYYY')}</Text>
                        </View>
                        <TouchableOpacity style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}
                            onPress={() => this.props.handoverSetProps({ isRefreshing: true, data: [] })}>
                            <MyIcon name={"layers"} size={100} color={colors.grayBorder} />
                            <Text style={{ fontWeight: 'bold', color: '#cccccc', fontSize: fontsize.small, textAlign: 'center', marginHorizontal: 20 }}>{Strings.app.error}</Text>
                        </TouchableOpacity>
                ...
                <View style={{
                    ...
                    <View style={{
                        ...
                        <TouchableOpacity style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}
                            onPress={() => this.props.handoverSetProps({ isRefreshing: true, data: [] })}>
                            <MyIcon name={"layers"} size={100} color={colors.grayBorder} />
                            <Text style={{ fontWeight: 'bold', color: '#cccccc', fontSize: fontsize.small, textAlign: 'center', marginHorizontal: 20 }}>{Strings.app.emptyData}</Text>
                        </TouchableOpacity>
                        {/* <ErrorContent title={Strings.app.emptyData} 
                onTouchScreen={() => this.props.handoverSetProps({ isRefreshing: true, data: [] })}
            /> */}
            ...
            <View style={{
                ...
                <View style={{
                    ...
                    <FlatList
                        ...
                        onRefresh={() => this.props.handoverSetProps({ isRefreshing: true, data: [] })}
        ...
        return (
            ...
            >
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    <View style={{
                        height: 10, width: 25, backgroundColor: '#f4ad41', marginRight: 5, marginLeft: -5, borderTopEndRadius: 10, borderBottomEndRadius: 10
                    }}></View>
                    {/* <Icon name='arrow-bold-right' size={ 20 } color='#f4ad41' style={{ marginRight: 5, marginLeft: -5 }}/> */}
                    <Text style={{ fontSize: fontsize.small, fontWeight: 'bold', color: '#000' }} >{moment(dateHandoverFrom).format('HH:mm')} - {moment(dateHandoverTo).format('HH:mm')}</Text>
                </View>
                            ...
                            <View style={{ flex: 1 }}>
                                <View style={{ flexDirection: 'row', alignItems: 'center', }}>
                                    <View
                                        ...
                                        <Icon2 name='home' size={20} color={colors.gray1} style={{}} />
                                    </View>

                                    <View>
                                        <Text style={{ fontSize: fontsize.small, fontWeight: 'bold', color: '#000', alignItems: 'center' }}>{apartmentName}</Text>
                                        <Text style={{ fontSize: fontsize.small, color: colors.gray1 }} >{buildingChecklistName}</Text>
                                        <Text style={{ fontSize: fontsize.small, color: colors.gray1, fontStyle: 'italic' }} >{scheduleConfirmName == null || scheduleConfirmName == '' ? 'Chưa nhận' : scheduleConfirmName}</Text>
                                    </View>
                                </View>
        ...
        return (
            <View style={styles.container}>
                <NavBar
                    ...
                    body={<Text style={{ alignSelf: 'center', color: '#fff', fontSize: fontsize.medium }}>{Strings.handoverSchedule.title.toLocaleUpperCase()}</Text>}
                    rightView={<TouchableOpacity onPress={() => this.props.refreshDataHandle()} style={{ padding: 10 }}><Icon name="cycle" color="#fff" size={20} /></TouchableOpacity>}
                />
...
export default connect(mapStateToProps, mapActionToState)(HandoverScheduleScreen);
