...
import { Alert, View, Text, Linking, StyleSheet, ScrollView, Platform, ActivityIndicator, TouchableOpacity } from 'react-native';
...
class NewsDetail extends Component {
    ...
    async loadDataHandle() {
        ...
        const ret = await get('/Residents/NewsDetail', { newsId: id, towerId, typeNews: type });
        console.log(ret);
        ...
        if (this.state.errors && this.state.errors.hasError) {
            return (
                ...
                >
                    ...
                    <ErrorContent
                        logo="layer" title={Strings.app.error + ' hoặc tin đã bị xóa!'} visibleButton={false} />
                    <View style={{ flexDirection: 'row', alignSelf: 'center', marginBottom: 50 }}>

                        <Text style={{ color: '#cccccc', marginLeft: 10, fontSize: fontsize.small }}>{Strings.app.touchToRefresh}</Text>
                    </View >
                ...
                <View style={{ flex: 1, marginHorizontal: 20 }} >
                    ...
                    <WebView
                        ...
                        source={{
                            ...
                            <img src=${this.state.data.imageUrl} style="border-radius: 20px !important">
                            </div>
                            <div style='color: #000000; font-size:16pt; margin-top: 10px; font-family: Asap-SemiBold'>${this.state.data.title}</div>
                            <div style='font-size: 12px; color: #6f6f6f; font-weight: normal; margin-bottom: 10; font-family: Asap-Regular'>${moment(this.state.data.dateCreate).format('DD/MM/YYYY HH:mm')}</div>
                        ...
                        {this.state.data.link.length > 0 &&
                            <TouchableOpacity
                                ...
                                <MyIcon
                                    ...
                                    color="#fff"
                                />
                                <Text style={{ color: '#fff', marginLeft: 10, fontSize: fontsize.small }}>Liên kết</Text>
                            </TouchableOpacity>}
                        {this.state.data.fileUrl.length > 0 &&
                            <TouchableOpacity
                                style={{ flex: 1, height: 50, flexDirection: 'row', backgroundColor: colors.appTheme, alignItems: 'center', justifyContent: 'center' }}
                                onPress={() => {
                                    try {
                            ...
                            >
                                <MyIcon
                                    ...
                                    color="#fff"
                                />
                                <Text style={{ color: '#fff', marginLeft: 10, fontSize: fontsize.small }}>{Strings.app.download}</Text>
                            </TouchableOpacity>}
                    </View>
            ...
            return (<View style={styles.container}>
                ...
                <NavBar
                    ...
                    body={<Text style={{
                        ...
                        textAlign: "center",
                        color: "#ffffff"
                    }}>{this.props.navigation.state.params.item.towerName.toLocaleUpperCase()}</Text>}
                />
                <View
                    style={{
                        paddingVertical: 20,
    ...
    download(url) {
        ...
        try {
            ...
            let options = {
                ...
                path: dirs.DCIMDir + "/file_" + Math.floor(date.getTime() + date.getSeconds() / 2) + ext
            }
...
export default connect(null, mapActionToProps)(NewsDetail);
