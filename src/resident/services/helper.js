...
export async function get(url, params = null) {
  ...
  await apiStorage.get(keys.idNew).then(async (data) => {
    //return -1;
    // console.log(data);
    if (data.status === 1) {
  ...
  try {
    if (helper.URL_API == "https://apimyhome.dip.vn/api") {
      ...
      let ret = await axios.get(
        `${helper.URL_API}` + url + `?idNew=${idNew}` + `&isPersonal=true`,
        {
          method: "GET",
          headers: {
            Authorization: `bearer ${token}`,
...
export async function get2(url, params = null) {
  //console.log(`${helper.URL_API}` + url)
  //console.log('params', params)
  await apiStorage.get(keys.idNew).then(async (data) => {
    //return -1;
...
export async function getVersion(url, params = null) {
  try {
    if (helper.URL_API == "https://apimyhome.dip.vn/api") {
      ...
      let ret = await axios.get(
        `${helper.URL_API}` + url + `?isPersonal=true`,
        {
          method: "GET",
        }
      );
  ...
  try {
    if (helper.URL_API == "https://apimyhome.dip.vn/api") {
      let ret = await axios.post(
        `${helper.URL_API}` + url + `?idNew=${idNew}`,
        params,
        {
          headers: {
