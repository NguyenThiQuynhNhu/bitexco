import { get, post, helper, post2 } from "../services/helper";
import {
...
export const loadDataHandle = (dataRequest) => async (dispatch) => {
  try {
    ...
    const ret = await post2(url, dataRequest.Id);
    if (ret !== undefined && ret !== null) {
      if (ret.status == 200) {
        console.log("ret data thanh cong", ret.message);
        //
        dispatch({
        ...
        console.log("ret data fail", ret.message);

        dispatch({
          type: CHECKLIST_TAISAN_GETLIST_FAILURE,
    ...
    console.log("ret fail", ret.message);
...
export const HandoverHandle = (dataRequest) => async (dispatch) => {
  try {
    ...
    // return
    const ret = await post2(url,dataRequest.id , parrams);
    //console.log(ret)
    if (ret !== undefined && ret !== null) {
      if (ret.status == 200) {
        //
        ...
        console.log("ret data", ret.message);

        dispatch({
          type: CHECKLIST_TAISAN_HANDOVER_FAILURE,
          payload: { data: ret.message },
        });
        return false;
      }
      ...
      try {
        ...
        if (ret !== undefined && ret !== null) {
          if (ret.status == 200) {
            ...
            dispatch({
              type: CHECKLIST_TAISAN_HANDOVER_UPLOAD_FILE_FAILURE,
              payload: { data: ret.message },
            });
            // return false
            item.status = JSON.parse(ret.data).status;
  ...
  try {
    ...
    if (ret !== undefined && ret !== null) {
      if (ret.status == 200) {
        ...
        dispatch({
          type: CHECKLIST_TAISAN_HANDOVER_DELETE_FILE_FAILURE,
          payload: { data: ret.message },
        });
        return false;
      }
  ...
  try {
    ...
    if (ret !== undefined && ret !== null) {
      if (ret.status == 200) {
        ...
        dispatch({
          type: CHECKLIST_TAISAN_CHECKLIST_ADD_FAILURE,
          payload: { data: ret.message },
        });
        return {
          status: 2,
  ...
  try {
    ...
    if (ret !== undefined && ret !== null) {
      if (ret.status == 200) {
        ...
        dispatch({
          type: CHECKLIST_ADD_DATA_FAILURE,
          payload: { data: ret.message },
        });
        return {};
      }
