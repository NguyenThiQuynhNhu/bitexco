...
import ListData from '../../../components/ShiftChange/List/ListData';
...
// create a component
class ShiftChangeList extends Component {
    constructor(props) {
        super(props);
        this.state = {
            depSelected: null,
    ...
    componentWillReceiveProps(nextProps) {
        const { isMine, towerId, rowPerPage, isRefreshing, initList, createStatus, errorResponse, refreshDataHandle, depSelected } = this.props;
            ...
            <View style={styles.container}>
                <NavBar
                    ...
                    body={<View style={{ justifyContent: 'center' }}>
                        <Text style={{ color: '#fff', alignSelf: 'center', fontSize: fontSize.medium, fontWeight: 'bold' }}>DANH SÁCH ĐỔI CA</Text>
                        <Text style={{ color: '#fff', alignSelf: 'center', fontSize: fontSize.small, fontWeight: 'bold' }}>Trạng thái: { converStatusToString(statusId) }</Text>
                    </View>
                    }
                ...
                {showFilter && <View style={{ ...Device.defaultMarginTop(), height: '100%', width: '100%', position: 'absolute', backgroundColor: colors.appOverView }}>
                    <View style={{ width: '90%', margin: 20, alignSelf: 'center', padding: 10, backgroundColor: '#fff', justifyContent: 'space-between' }}>
                        ...
                        />
                        <TouchableOpacity
                            onPress={() => this.props.navigation.navigate('proposalStatus', { id: user.towerId, onSelected: (depSelected) => this.setState({ depSelected, statusId: depSelected.id }) })}
                            style={{ padding: 10, marginTop: 10, borderRadius: 5, backgroundColor: colors.gray2, flexDirection: 'row', justifyContent: 'space-between' }}>
                            <Text>{this.props.depSelected ? this.props.depSelected.name : depSelected ? depSelected.name : 'Chọn trạng thái'}</Text>
                            <MyIcon
                                name="arrow-down"
                                size={20}
                                color={colors.gray1}
                        ...
                        <View style={{ flexDirection: 'row', marginTop: 20, justifyContent: 'center' }}>
                            <PrimaryButton text="Bỏ lọc"
                                onPress={() => this.setState({ searchKey: '', isApplySearchKey: false, showFilter: false, depSelected: null, statusId: -1 }, () => onClearFilter())}
                            />
                            <PrimaryButton
...
const mapStateToProps = (state) => ({
    ...
    initList: state.shiftChange.initList,
    currentPage: state.shiftChange.currentPage,
    rowPerPage: state.shiftChange.rowPerPage,
    emptyData: state.shiftChange.emptyData,
    outOfStock: state.shiftChange.outOfStock,
    isLoading: state.shiftChange.isLoading,
    data: state.shiftChange.data,
    error: state.shiftChange.error,
    isRefreshing: state.shiftChange.isRefreshing,
    isApplySearchKey: state.shiftChange.isApplySearchKey,
    searchKey: state.shiftChange.searchKey,
    createStatus: state.shiftChange.createStatus,
    canNavigate: true,//state.requestDetail.data == null,
    language: state.app.language,
    isMine: state.shiftChange.isMine,
    user: state.auth.user,
    statusId: state.shiftChange.statusId,
    //errorResponse: state.requestDetail.errorResponse,
    towerId: state.auth.user.towerId,
    depSelected: state.shiftChange.depSelected
});
...
export default connect(mapStateToProps, mapActionToState)(ShiftChangeList);
