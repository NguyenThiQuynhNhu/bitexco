...
class GroupWorkingStatisticsScreen extends React.Component {
        ...
        return (
            < ScrollView style={{ flex: 1, marginTop: 10 }}>
                ...
                <View style={{ backgroundColor: '#fff', margin: 10, borderRadius: 12, }}>
                    ...
                    <ReportGroupProgressStatus dateFrom={this.state.startDate} dateTo={this.state.endDate} />
                    <FlatList
                        ...
                        renderItem={(item) => {
                            return (
                                <TouchableOpacity
                                    style={{ width: '50%' }}
                                    onPress={() => { this.props.navigation.navigate('requests', { idStatus: item.item.id }) }}
                                >
                                    <View
                                        style={{
                                            borderRadius: 45,
                ...
                <View style={{ backgroundColor: '#fff', margin: 10, borderRadius: 12, }}>
                    <View style={{ flexDirection: 'row', padding: 10, alignItems: 'center', justifyContent: 'space-between' }}>
                        ...
                        }}>Thời gian xử lý trung bình</Text>
                    </View>
                    <ReportGroupProgressTimeComplete dateFrom={this.state.startDate} dateTo={this.state.endDate} />
                </View>
                ...
                <View style={{ backgroundColor: '#fff', margin: 10, borderRadius: 12, }}>
                    <View style={{ flexDirection: 'row', padding: 10, alignItems: 'center', justifyContent: 'space-between' }}>
                        <Text style={{
                            ...
                            fontWeight: "600",
                            color: "#282828", paddingRight: 10
                        }}>Điểm đánh giá trung bình</Text>
                    </View>
                    <ReportGroupProgressRating dateFrom={this.state.startDate} dateTo={this.state.endDate} />
                </View>
            </ScrollView >)
            ...
            <View style={{ backgroundColor: '#eeeeee', flex: 1 }}>
                ...
                <View style={{ flex: 1 }}>
                    ...
                    <View style={{ backgroundColor: '#fff', padding: 10 }}>
                        ...
                        <Text style={{ alignSelf: 'center', fontSize: fontsize.small }}>Từ <Text style={{ color: '#0084ff', fontSize: fontsize.small }}>{moment(this.state.startDate).format('DD-MM-YYYY')}</Text> đến <Text style={{ color: '#0084ff', fontSize: fontsize.small }}>{moment(this.state.endDate).format('DD-MM-YYYY')}</Text></Text>
                    </View>
                    {this.renderContent()}
                    <ModalPicker
                        dislayValue="value"
                        ...
                        selectedValue={0}
                        onValueChange={(typeTime) => this.setState({ showModalPicker: false, typeTime }, () => { this.refreshDataHandle() })}
                        data={DataFilter}
    ...
    refreshDataHandle() {
        const { refreshDataHandle, timeRefreshDataHandle, ratingRefreshDataHandle } = this.props
        refreshDataHandle();
        timeRefreshDataHandle();
        ratingRefreshDataHandle();
    }
...
export default connect(mapStateToProps, mapActionToProps)(GroupWorkingStatisticsScreen);
