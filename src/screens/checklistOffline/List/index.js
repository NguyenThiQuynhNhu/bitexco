...
class ChecklistOfflineList extends Component {
  ...
  componentDidMount = async () => {
    ...
    await apiStorage.get(keys.keyChecklists).then(async (data) => {
      //console.log(data);
      if (data.status === 1) {
  ...
  renderItem = ({ item, index }) => {
    ...
    return (
      <TouchableOpacity
        ...
        onPress={() =>
          this.props.navigation.navigate("checklisOfflinetDetail", item)
        }
      >
        <View
          style={{
