...
import { ImageBackground, View, Text, StyleSheet, TouchableOpacity, FlatList, ScrollView, ActivityIndicator, Platform, SafeAreaView, Modal, Image } from 'react-native';
...
import { getProfile, postFCMToken, delFCMTokenEmployee } from '../../resident/actions/auth';
import { loadDataHandle } from '../../resident/actions/vendorHome';
import { loadDataHandle as loadDataHandleNotify, resetStateByKey, refreshDataHandle, onSubmitEditing, onChangeText, onClearText } from '../../resident/actions/news';
...
class NewsScreen extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        if (nextProps.initListNotify && nextProps.initListNotify !== this.props.initListNotify) {
            //console.log('1.')
            const data = {
                towerId: this.props.towerId,
                keyword: nextProps.isApplySearchKeyNotify ? nextProps.searchKeyNotify : '',
        ...
        if (nextProps.isRefreshingNotify && nextProps.isRefreshingNotify !== this.props.isRefreshingNotify && !nextProps.isLoadingNotify) {
            //console.log('2.')
            const data = {
                towerId: this.props.towerId,
                keyword: nextProps.isApplySearchKeyNotify ? nextProps.searchKeyNotify : '',
    ...
    renderItem = ({ item }) => {
        const { id, dateCreate, imageUrl, shortDescription, totalRead, title } = item
        return (
            <TouchableOpacity
                onPress={() => this.props.navigation.navigate('newsDetail', { item: { ...item, towerId: this.props.towerId, title: title, towerName: this.props.user.towerName }, type: 1 })}
                style={{
                    borderRadius: 12,
    ...
    renderContent() {
        const { isLoading, data, error, initComponent, user, dataNotify, isRefreshingNotify, refreshDataHandle, emptyDataNotify, errorNotify } = this.props

        if (errorNotify && errorNotify.hasError) {
            return (
                <ErrorContent title={Strings.app.error} onTouchScreen={() => {
        ...
        if (emptyDataNotify) {
            return (
                <ErrorContent title={Strings.app.emptyData} onTouchScreen={() => {
                    this.props.getProfile({ type: 're', langId: this.props.langId }),
                        this.props.refreshDataHandle()
                }} />
            )
        ...
        return (
            <FlatList
                ...
                onRefresh={() => { refreshDataHandle(), this.props.getProfile({ type: 're', langId: this.props.langId }) }}
                data={dataNotify || []}
                ...
                onEndReached={() => {
                    if (!this.onEndReachedCalledDuringMomentum
                        ...
                        const data = {
                            towerId: this.props.towerId,
                            keyword: this.props.isApplySearchKeyNotify ? this.props.searchKeyNotify : '',
    ...
    renderFooter = () => {
        if (!this.props.isLoadingNotify || this.props.isRefreshingNotify) return null;
        return (
            <View
                style={{
                    paddingVertical: 20,
...
export default connect(mapStateToProps, mapActionToState)(NewsScreen);
