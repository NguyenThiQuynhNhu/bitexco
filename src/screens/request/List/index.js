...
import { converStatusToColor, converStatusToString, converTypeToSource } from '../../../utils/request';
...
class ReuqestList extends Component {
    ...
    componentDidMount() {
        ...
        if(this.props.navigation.state.params && this.props.navigation.state.params.isMine)
    ...
    async _onValueChangeSwitch() {
        await this.props.resetStateByKey({ key: 'isMine', path: '', value: !this.props.isMine });
        this.props.dataStatus.forEach(element => {
            element.currentValue = this.state.status
            //ar.push(element)
        });
            ...
            <View style={styles.container}>
                <NavBar
                    ...
                    body={<View style={{ justifyContent: 'center' }}>
                        ...
                        <Text style={{ color: '#fff', alignSelf: 'center', fontSize: fontSize.small, fontWeight: 'bold', fontFamily: "Inter-Bold" }}>Nguồn: {converTypeToSource(typeId)}</Text>
                    </View>
                    }
                    rightView={<View style={{ flex: 1, flexDirection: 'row' }}>
                        <View style={{ alignItems: 'center', flexDirection: 'row', marginLeft: 10 }}>
                            <Text style={{
                                marginRight: 10, fontFamily: "Inter-Bold",
                                fontSize: 14,
                                fontWeight: "bold",
                                fontStyle: "normal",
                                letterSpacing: 0,
                ...
                {showFilter && <View style={{ ...Device.defaultMarginTop(), height: '100%', width: '100%', position: 'absolute', backgroundColor: colors.appOverView }}>
                    <View style={{ width: '90%', margin: 20, alignSelf: 'center', padding: 10, backgroundColor: '#fff', justifyContent: 'space-between' }}>
                        ...
                        />
                        <TouchableOpacity
                            onPress={() => this.props.navigation.navigate('depDictionary', { id: user.towerId, onSelected: (depSelected) => this.setState({ depSelected }) })}
                            style={{ padding: 10, marginTop: 10, borderRadius: 5, backgroundColor: colors.gray2, flexDirection: 'row', justifyContent: 'space-between' }}>
                            <Text style={{ fontSize: fontSize.small }}>{this.props.depSelected ? this.props.depSelected.name : depSelected ? depSelected.name : 'Chọn phòng ban'}</Text>
                            <MyIcon
                                name="arrow-down"
                                size={20}
                                color={colors.gray1}
                        ...
                        <View style={{ flexDirection: 'row', marginTop: 10, marginLeft: -10 }}>
                            <ButtonDateFilter value={typeId == 0} text="Tất cả" onPress={() => this.setTypeActive(0)} />
                            <ButtonDateFilter value={typeId == 1} text="Từ cư dân" onPress={() => this.setTypeActive(1)} />
                            <ButtonDateFilter value={typeId == 2} text="Từ vận hành" onPress={() => this.setTypeActive(2)} />
                        </View>

                        <View style={{ flexDirection: 'row', marginTop: 20, justifyContent: 'center' }}>
                            <PrimaryButton text="Bỏ lọc"
                                onPress={() => this.setState({ searchKey: '', isApplySearchKey: false, showFilter: false, depSelected: null }, () => onClearFilter())}
                            />
                            <PrimaryButton
    ...
    _onSelectedChange = async (value) => {
        if (value == this.state.status) {
            ...
            await this.setState({ status: value }, () => this.props.refreshDataHandle())
            const ar = [];
            await this.props.dataStatus.forEach(element => {
...
export default connect(mapStateToProps, mapActionToState)(ReuqestList);
