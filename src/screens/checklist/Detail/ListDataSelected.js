...
class ListDep extends Component {
    ...
    render() {
        ...
        const { sourceId } = this.props.navigation.state.params;
        return (
            <View
                style={styles.container}
            >

                <NavBar leftButton={leftButton} body={<Text style={{ color: '#fff', fontSize: fontsize.medium, fontWeight: 'bold', alignSelf: 'center' }}>{'Chọn'}</Text>} rightView={null} />

                <ListData
                    api={{
                        url: '/Vendors/Checklist/Source',
                        params: {
                            sourceId
                        },
                        loadmore: false
