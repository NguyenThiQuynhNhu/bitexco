...
class ChecklistList extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        ...
        const { isMine, towerId, rowPerPage, isRefreshing, initList, createStatus, errorResponse, refreshDataHandle, statusSelected } = this.props;
        if (nextProps.isRefreshing && nextProps.isRefreshing !== isRefreshing) {
            const data = {
                ...
                rowPerPage,
                departmentId: nextProps.statusSelected ? nextProps.statusSelected.id : 0,
    ...
    _renderContent() {
        const { emptyData, error, initList, data, isRefreshing, outOfStock, refreshDataHandle, loadDataHandle, isLoading, statusSelected } = this.props;
        //console.log(data)
        ...
        return (
            <View style={styles.container}>
                <NavBar
                    ...
                    body={<View style={{ justifyContent: 'center' }}>
                        <Text style={{ fontFamily: "Inter-Bold",
                            ...
                            letterSpacing: 0,
                            textAlign: "center",
                            color: "#ffffff" }}>{ this.state.isMaintenance ? 'Kiểm tra định kỳ' : 'Checklist'}</Text>
                    </View>
                    }
                ...
                <View>
                    ...
                    >
                        <ButtonFilter value={0} currentValue={this.state.status} onSelectedChange={this._onSelectedChange} style={{ borderRightWidth: 1, borderColor: colors.grayBorder }} />
                        <ButtonFilter value={1} currentValue={this.state.status} onSelectedChange={this._onSelectedChange} style={{ borderRightWidth: 1, borderColor: colors.grayBorder }}/>
                        <ButtonFilter value={2} currentValue={this.state.status} onSelectedChange={this._onSelectedChange} style={{ borderRightWidth: 1, borderColor: colors.grayBorder }}/>
                        <ButtonFilter value={3} currentValue={this.state.status} onSelectedChange={this._onSelectedChange} style={{ borderRightWidth: 1, borderColor: colors.grayBorder }}/>
                ...
                {showFilter && <View style={{ ...Device.defaultMarginTop(), height: '100%', width: '100%', position: 'absolute', backgroundColor: colors.appOverView }}>
                    <View style={{ width: '90%', margin: 20, alignSelf: 'center', padding: 10, backgroundColor: '#fff', justifyContent: 'space-between' }}>
                        ...
                        />
                        <TouchableOpacity
                            onPress={() => this.props.navigation.navigate('checklistStatus', { id: user.towerId, onSelected: (statusSelected) => this.setState({ statusSelected, status: statusSelected.id }) })}
                            style={{ padding: 10, marginTop: 10, borderRadius: 5, backgroundColor: colors.gray2, flexDirection: 'row', justifyContent: 'space-between' }}>
                            <Text>{this.props.statusSelected ? this.props.statusSelected.name : statusSelected ? statusSelected.name : 'Chọn trạng thái'}</Text>
                            <MyIcon
                                name="arrow-down"
                                size={20}
                                color={colors.gray1}
                        ...
                        <View style={{ flexDirection: 'row', marginTop: 20, justifyContent: 'center' }}>
                            <Button text="Bỏ lọc"
                                onPress={() => this.setState({ searchKey: '', isApplySearchKey: false, showFilter: false, statusSelected: null, status: -1 }, () => onClearFilter())}
                            />
                            <Button
                                visible={this.state.searchKey !== '' || this.props.statusSelected !== null || statusSelected !== null}
                                onPress={() => this.setState({ showFilter: false, isApplySearchKey: this.state.searchKey.length !== 0 }, () => onFilter({ statusSelected }))}
                                text="Lọc dữ liệu"
                                style={{ marginLeft: 10 }} />
                        </View>
...
export default connect(mapStateToProps, mapActionToState)(ChecklistList);
