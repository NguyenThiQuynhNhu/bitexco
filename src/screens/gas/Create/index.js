...
//
import {
    loadDataHandle, loadDataUnitHandle,
    resetStateByKey,
    createHandle
...
class CreateGasScreen extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            gasRequest: props.navigation.state.params.data.indexId > 0 ?
                {
                    ...
                    this.state.gasRequest.linkUrl != '' ?
                        <View>
                            ...
                            /> */}
                                ...
                                >
                                    <Image source={{ uri: this.state.gasRequest.linkUrl }} style={{ width: 90, height: 120, zIndex: 0, borderRadius: 8 }} />
                                </Lightbox>
                            </View>
                        </View>
                        :
                    ...
                    <View style={styles.container}>
                        <View style={{
                            ...
                            <View style={{
                                ...
                                <TextInput
                                    ...
                                    onChangeText={(indexNew) => { this.setState({ gasRequest: { ...this.state.gasRequest, indexNew } }) }}
                                />
                            </View>
                            ...
                            <View style={{
                                ...
                                <TouchableOpacity
                                    onPress={() => this.props.navigation.navigate('unitList', { units: dataUnit, onSelected: (unitSelected) => this.setState({ unitSelected }) })}
                                    style={{
                                        padding: 10, borderRadius: 5, borderWidth: 1,
                                        borderColor: "#cbcbcb", flexDirection: 'row', justifyContent: 'space-between'
                                    }}>
                                    <Text style={{ paddingRight: 10, fontSize: fontsize.small }}>{this.state.unitSelected ? this.state.unitSelected.name : 'Chọn đơn vị tính'}</Text>
                                    <MyIcon
                                        name="arrow-down"
                                        size={14}
                                        color={colors.gray1}
                            ...
                            <View style={{
                                ...
                                <TextInput
                                    ...
                                    underlineColorAndroid='transparent'
                                    onChangeText={(rateFeeVAT) => { this.setState({ gasRequest: { ...this.state.gasRequest, rateFeeVAT } }) }}
                                />
                            ...
                            <View style={{
                                ...
                                <Text style={{ color: colors.appTheme, fontSize: fontsize.small }}>Ghi chú</Text>
                                <TextInput
                                    maxLength={500}
                                    underline={false}
                                    multiline
                                    ...
                                    placeholderTextColor="#a0a0a0"
                                    value={description}
                                    onChangeText={(description) => { this.setState({ gasRequest: { ...this.state.gasRequest, description } }) }}
                                />
                            </View>
                        </View>
                        ...
                        <View style={{
                            ...
                            <Text style={{
                                ...
                                fontWeight: "600",
                                color: "#f53b3b"
                            }}>{this.state.unitSelected ? this.state.unitSelected.tyGia : ''}</Text>
                        </View>
                        <TouchableOpacity
                ...
                >
                    <View style={{ flex: 1, backgroundColor: colors.appOverView, justifyContent: 'center', alignItems: 'center' }}>
                                ...
                                <View style={{ flex: 1, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', borderWidth: 0 }}>
                                    <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center' }}>
                                        ...
                                        <TouchableOpacity onPress={() => this._showDateTimePicker(true)} bordered dark style={{ borderRadius: 0, flex: 1, justifyContent: 'center', alignItems: 'center', marginRight: 5, padding: 10 }}>
                                            <Text style={{ fontSize: fontSize.medium, color: '#111' }}>{moment(this.state.gasRequest.dateFrom).format('DD-MM-YYYY')}</Text>
                                        </TouchableOpacity>
                                        ...
                                        <TouchableOpacity onPress={() => this._showDateTimePicker(false)} bordered dark style={{ borderRadius: 0, flex: 1, padding: 10, justifyContent: 'center', alignItems: 'center' }}>
                                            <Text style={{ fontSize: fontSize.medium, color: '#111' }}>{moment(this.state.gasRequest.dateTo).format('DD-MM-YYYY')}</Text>
                                        </TouchableOpacity>
                                    </View>
                        ...
                        <DateTimePicker
                            ...
                            date={this.state.isStartDate ? new Date(this.state.gasRequest.dateFrom) : new Date(this.state.gasRequest.dateTo)}
                        />
                    </View>
    ...
    _handleDatePicked = (date) => {
        if (this.state.isStartDate) {
            this.setState({ gasRequest: { ...this.state.gasRequest, dateFrom: moment(date).format('YYYY-MM-DD') } });
        } else {
            this.setState({ gasRequest: { ...this.state.gasRequest, dateTo: moment(date).format('YYYY-MM-DD') } });
        }
        this._hideDateTimePicker();
    };
    _handleDatePickedThanhToan = (date) => {
        this.setState({ gasRequest: { ...this.state.gasRequest, datePayment: moment(date).format('YYYY-MM-DD') } });
        this._hideDateTimePicker();
    };
    _handleDatePickedThongBao = (date) => {
        this.setState({ gasRequest: { ...this.state.gasRequest, dateNotify: moment(date).format('YYYY-MM-DD') } });
        this._hideDateTimePicker();
    };
    customTime = () => {
        this.setState({
...
export default connect(mapStateToProps, mapActionToProps)(CreateGasScreen);
