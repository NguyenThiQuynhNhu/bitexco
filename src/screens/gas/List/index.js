                ...
                <NavBar
                    ...
                    body={<View style={{ justifyContent: 'center' }}>
                        ...
                        <Text style={{ fontFamily: "Inter-Medium",
                            ...
                            letterSpacing: 0,
                            textAlign: "left",
                            color: "#ffffff" }}>{this.props.user.towerName.toLocaleUpperCase()} - Kỳ: {searchKey}/{searchKey2}</Text>
                    </View>
                    }
                    // rightView={<TouchableOpacity
...
<View style={{ borderTopRightRadius: 20 }}>
                    <View style={{ backgroundColor: '#fff', justifyContent: 'space-between', padding: 10, marginHorizontal: 20,
                        borderRadius: 20,
                ...
                {showFilter && <View style={{ ...Device.defaultMarginTop(), flex: 1, height: '100%', width: '100%', position: 'absolute', backgroundColor: colors.appOverView }}>
                    <View style={{ width: '90%', margin: 20, alignSelf: 'center', padding: 10, backgroundColor: '#fff', justifyContent: 'space-between' }}>
                        ...
                        <View style={{ flexDirection: 'row', marginTop: 20, justifyContent: 'center' }}>
                            <PrimaryButton text="Bỏ lọc"
                                onPress={() => this.setState({ searchKey: new Date().getMonth() + 1,
                                    searchKey2: new Date().getFullYear(), isApplySearchKey: false, showFilter: false, blockSelected: null, floorSelected: null, statusId: -1 }, () => onClearFilter())}
                            />
                            <PrimaryButton
                                visible={ this.props.floorSelected !== null || floorSelected !== null}
                                onPress={() => this.setState({ showFilter: false, isApplySearchKey: this.state.searchKey.length !== 0 }, () => onFilter({ blockSelected, floorSelected, 
                                    searchKey: this.state.searchKey, searchKey2: this.state.searchKey2, }))}
                                text="Lọc dữ liệu"
                                style={{ marginLeft: 10, fontWeight: 'bold' }} /> 
                        </View>
                ...
                onPress={() => {
                    switch(item.id){
                        ...
                        case 1://Cập nhật, thêm
                            this.setState({ showAction: false }, () => {
                                this.props.navigation.navigate('gasDetail', { data: this.props.itemSelected })
                            })
                        break;
...
export default connect(mapStateToProps, mapActionToState)(GasList);
