...
import { demo, getOtpCodeHaveType, loginUser, changePhone, resetStateByKey, loginUserByPass, getOtpCodeNoType, setTypeResident, setTypeVendor, loginUserByPassVendor, loginUserByPassResident, signIn } from '../../actions/auth';
...
class LoginScreen extends Component {
    ...
    constructor(props) {
        super(props)
        this.state = {
            phoneNumber: this.props.navigation.state.params && this.props.navigation.state.params.phoneNumber ? this.props.navigation.state.params.phoneNumber : '',
            password: this.props.navigation.state.params && this.props.navigation.state.params.password ? this.props.navigation.state.params.password : '',
            otpCode: '',
            isPass: true,
            isRegister: false,
            passwordRetype: '',
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (nextProps && nextProps.error && nextProps.error.hasError && this.props.error !== nextProps.error) {
    ...
    // }
    renderBody() {
        const { navHome, getOtpCode, changePhone, loginUser, loginUserByPass, type } = this.props
        //console.log('this.props', this.props)
        const { phoneNumber, otpCode, isPass, password, isType, passwordRetype, isRegister } = this.state
            ...
            case 'LOGIN':
                return (
                    <BodyContent
                        ...
                        onChangePasswordRetype={(passwordRetype) => this.setState({ passwordRetype: passwordRetype })}
                        isPass={isPass}
                        goCase={this.goCase}
            ...
            case 'VERIFY':
                return (
                    <BodyContent
                        ...
                        onFulfill={(code) => loginUser({ phoneNumber, type, otpCode: code })}
                    />
                )
            case 'SELECT_TYPE':
                return (
                    <BodyContent
                        ...
                        onFulfill={(code) => this.setState({ choiceType: true, otpCode: code })}
                    />
                )
    ...
    render() {
        ...
        return (
            <View>
                ...
                <Image
                    source={require('../../resources/Vector.png')}
                    style={{
                        width: Screen.width,
    ...
    _onRequestOTPResident = () => {
        ...
        // }
            ...
            if (isPass) {
                this.props.loginUserByPassResident({ phoneNumber, password, type: 're', idNew: this.props.idNew })
            } else {
                if (otpCode.length == 0) {
                    Alert.alert(Strings.tabbar.notification, Strings.login.emptyOtp);
                } else {
    ...
    _onRequestOTPVendor = () => {
        ...
        // }
            ...
            if (isPass) {
                this.props.loginUserByPassVendor({ phoneNumber, password, type: 'em', idNew: this.props.idNew })
            } else {
                if (otpCode.length == 0) {
                    Alert.alert(Strings.tabbar.notification, Strings.login.emptyOtp);
                } else {
    ...
    _onLoginBypass = async () => {
        ...
        const isPersonal = helper.URL_API == 'https://apimyhome.dip.vn/api' ? false : true;
        if (phoneNumber.length == 0 || password.length == 0) {
            Alert.alert(Strings.tabbar.notification, Strings.login.emptyPass);
            this.setState({ isLoading: false })
        } else {
            try {
                let ret = await axios.post(`${helper.URL_API}/User/GetBuilding?isPersonal=${isPersonal}`,
                    {
                    ...
                    if (ret.status == 200) {
                        if (ret.data.length > 0) {
                            if (ret.data.length == 1) {
                                this.setState({ isLoading: false })
                                this.props.loginUserByPass({ phoneNumber: phoneNumber, password: password, idNew: ret.data[0].id })
                            } else {
                                //console.log(this.state.isLoading)
                                this.setState({ isLoading: false })
                                this.props.navigation.navigate('building', { phoneNumber, password })
                            }
                        } else {
                            this.setState({ isLoading: false })
    ...
    _onRegister = () => {
        ...
        if (phoneNumber.length == 0 || password.length == 0 || passwordRetype.length == 0) {
