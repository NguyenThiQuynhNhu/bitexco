...
import { View, Text, StyleSheet, WebView, Platform, ActivityIndicator, TouchableOpacity } from 'react-native';
    ...
    render() {
        ...
        if (this.state.errors && this.state.errors.hasError) {
            return (
                ...
                >
                    <ErrorContent
                        logo="layer" title="LỖI KẾT NỐI" visibleButton={false} />
                    <View style={{ flexDirection: 'row', alignSelf: 'center', marginBottom: 50 }}>

                        <Text style={{ color: '#cccccc', marginLeft: 10 }}>Chạm để thử lại</Text>
                    </View >
        ...
        if (this.state.emptyData) {
            return (
                <ErrorContent logo="layer" title="KHÔNG CÓ DỮ LIỆU" buttonText="Quay lại" onPressButton={() => this.props.navigation.goBack()} />
            );
        }
